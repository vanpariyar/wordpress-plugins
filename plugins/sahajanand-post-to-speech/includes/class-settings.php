...
class Sahajanand_Post_To_Speech_Settings {
	...
	public function register_menu() {
		add_options_page(
			__( 'Sahajanand Post to Speech', 'sahajanand-post-to-speech' ),
			__( 'Sahajanand Post to Speech', 'sahajanand-post-to-speech' ),
			'manage_options',
	...
	public function enqueue_admin_assets( $hook ) {
		if ( 'settings_page_sahajanand-post-to-speech' !== $hook ) {
			return;
		}
	...
	public function render_section() {
		...
		printf(
			wp_kses(
				...
				__( 'API mode requires a self-hosted KittenTTS HTTP API. See the %s project for details.', 'sahajanand-post-to-speech' ),
				array(
					'a' => array(
						'href'   => array(),
						'target' => array(),
	...
	public function render_model_field() {
		...
		$labels = array(
			...
			'KittenML/kitten-tts-micro-0.8'     => __( 'Micro (~41 MB)', 'sahajanand-post-to-speech' ),
		...
		echo '</select>';
		echo '<p class="description">' . esc_html__( 'Browser mode auto-uses the fp32 model when int8 is selected (int8 is unreliable in WASM). Downloaded on first use.', 'sahajanand-post-to-speech' ) . '</p>';
	}
